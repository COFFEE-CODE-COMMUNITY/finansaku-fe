...
function Survey() {
    ...
    <div className="flex flex-col relative justify-center ml-4 items-start text-white w-full h-full">
        ...
        <div className="flex flex-col gap-4 top-[150px] absolute justify-center items-start">
            ...
            <div className="mb-2.5 flex flex-col gap-4">
                <label className="font-semibold text-xl" htmlFor="statusUser"> 2. Status kamu saat ini?</label>
                <input type="text" placeholder="Rp. 5.000.000" onChange={(e) => (setStatusUser(e.target.value))} value={salaryUser} className={`${ errors.salaryUser ? "border-red-400" : "border-gray-400"} border w-[1000px] rounded p-2.5 focus:outline-none focus:border-blue-500`}/>
            ...
            <div className="mb-2.5 flex flex-col gap-4">
                <label className="font-semibold text-xl" htmlFor="salaryUser"> 4. Berapakah nominal gaji yang kamu terima setiap bulan?</label>
                <input type="text" placeholder="Rp.5.000.000" id="statusUser" onChange={(e) => (setSalaryUser(e.target.value))} value={salaryUser} className={`${ errors.statusUser ? "border-red-400" : "border-gray-400"} border w-[1000px] rounded p-2.5 focus:outline-none focus:border-blue-500`} />
                {errors.salaryUser && (errors.salaryUser)}
            </div>

            <div className="mb-2.5 flex flex-col gap-4">
                <label className="font-semibold text-xl" htmlFor="totalFamily">Berapa total anggota keluarga yang menjadi tanggungan kamu saat ini?</label>
                <input type="text" placeholder="Masukkan jumlah anggota keluarga (misal: 4)" id="totalFamily" onChange={(e) => (setTotalFamily(e.target.value))} value={totalFamily} className={`${ errors.statusUser ? "border-red-400" : "border-gray-400"} border w-[1000px] rounded p-2.5 focus:outline-none focus:border-blue-500`} />
                {errors.totalFamily && (errors.totalFamily)}
            </div>
        </div>
