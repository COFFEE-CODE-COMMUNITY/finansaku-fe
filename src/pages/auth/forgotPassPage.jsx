    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok){
                ...
                setErrors({ email: body.message || "Failed to send reset email." });
            }
    ...
    <div className="flex h-screen w-full text-white">
        <div className="flex flex-col justify-center items-center w-1/2 bg-white text-black gap-8 px-8">
            <div className="text-center flex flex-col gap-2.5">
                ...
                <p className="text-lg text-black">Page for sending reset links<br />via email when you forget your password</p>
            </div>
            ...
            <form className="flex flex-col gap-8 w-3/4 max-w-sm" onSubmit={handleSubmit} >
                <div className="flex flex-col text-left">
                    ...
                    <input type="email" name="userEmail" onChange={(e) => setEmail(e.target.value)} value={email}  placeholder="yourname@gmail.com" id="userEmail" className="border border-gray-400 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"  />
                ...
                <button type="submit" className="bg-[#1B263B] hover:bg-[#15224A] text-white font-semibold py-2 rounded-full transition flex justify-center items-center" disabled={loading}>
                    {loading ? (
                    <>
                        <Loader2 className="animate-spin mr-2 h-4 w-4" /> Sending the reset link
                    </>
                    ) : (
